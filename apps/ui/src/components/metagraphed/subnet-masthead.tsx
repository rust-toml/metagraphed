    ...
    <header className="mb-6">
      ...
      <div className="grid grid-cols-[auto_minmax(0,1fr)] md:grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-3 md:gap-4">
        ...
        <div className="min-w-0">
          {/* Title line — name + symbol only, with header actions pinned
              right. Keeping this to a single short row means the actions never
              steal width from (and crowd) the taxonomy row below. */}
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-baseline gap-2">
              ...
              {profile?.symbol ? (
                <span className="shrink-0 font-mono text-sm text-ink-muted">{profile.symbol}</span>
              ) : null}
            </div>
            {actions ? <div className="shrink-0">{actions}</div> : null}
          </div>
          {/* Taxonomy — type stays a single anchor pill; the categories collapse
              to one compact dot-separated metadata line so they read as a single
              row that wraps, instead of a tall stack of individual boxes. */}
          {profile?.subnet_type || categories.length > 0 ? (
